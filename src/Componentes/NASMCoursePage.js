  ...
  const filteredSections = useMemo(() => {
    ...
    return sections
      .map((s) => {
        ...
        const filteredContent = s.content.filter((item) => item.toLowerCase().includes(q));
        // si matchea título o al menos un bullet
        if (titleMatch || filteredContent.length > 0) {
          return {
            ...s,
            content: titleMatch ? s.content : filteredContent, // si título matchea, muestra todo
          };
        }
        return null;
      })
            ...
            <div className="sidebar">
              ...
              <div className="widget-progress">
                ...
                <div className="progress-row">
                  <span>{completed.length}/{sections.length} secciones</span>
                  <span>{progress}%</span>
                </div>
